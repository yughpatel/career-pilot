...
export default function Settings() {
  ...
  const loadAiSettings = async () => {
    try {
      ...
      if (aiConfigStr) {
        ...
        setValue('aiKey', decryptKey(config.apiKey) || '')
        setValue('aiModel', config.model || '')
      } else {
        const openRouterKey = localStorage.getItem('openRouterApiKey')
        if (openRouterKey) {
          setAiProvider('openrouter')
          setValue('aiKey', decryptKey(openRouterKey) || '')
        }
      }
    } catch (error) {
  ...
  const loadAiModels = async () => {
    ...
    try {
      ...
      if (res.success) setAiModelsList(res.models)
    } catch {
  ...
  const onSubmitAi = async (data) => {
    setSavingAi(true)
    try {
      const aiConfig = {
        provider: aiProvider,
        apiKey: encryptKey(data.aiKey),
        model: data.aiModel,
      }
      localStorage.setItem('aiConfig', JSON.stringify(aiConfig))
      if (aiProvider === 'openrouter' && data.aiKey) {
        localStorage.setItem('openRouterApiKey', encryptKey(data.aiKey))
      } else if (!data.aiKey) {
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="p-6 rounded-2xl bg-neutral-900/50 border border-neutral-800 space-y-6">
            ...
            <div className="flex items-center justify-between py-4 border-b border-neutral-800">
              ...
              <Toggle value={preferences.jobAlerts} onChange={(val) => setPreferences({ ...preferences, jobAlerts: val })} />
            </div>
            ...
            <div className="flex items-center justify-between py-4 border-b border-neutral-800">
              ...
              <Toggle value={preferences.directMessages} onChange={(val) => setPreferences({ ...preferences, directMessages: val })} />
            </div>
            ...
            <div className="flex items-center justify-between py-4">
              ...
              <Toggle value={preferences.proposalUpdates} onChange={(val) => setPreferences({ ...preferences, proposalUpdates: val })} />
            </div>

            <Button onClick={handleSave} loading={saving} variant="gradient" className="w-full flex items-center justify-center gap-2">
              <Save className="w-4 h-4" />
              Save Preferences
            </Button>
          </div>

          <form onSubmit={handleSubmit(onSubmitAi)} noValidate>
            <div className="p-6 rounded-2xl bg-neutral-900/50 border border-neutral-800 space-y-6 mt-8">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value !== 'openrouter') setValue('aiModel', '')
                      }}
                  ...
                  {aiProvider && (
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-neutral-800 border border-neutral-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-indigo-500"
                        {...register('aiKey', {
                          required: aiProvider ? 'API key is required' : false,
                        })}
                      />
                      {errors.aiKey && (
                        <p className="text-red-400 text-xs mt-1">{errors.aiKey.message}</p>
                      )}
                    </div>
                  )}
              ...
              <Button type="submit" variant="outline" loading={savingAi} className="w-full flex items-center justify-center gap-2 mt-4 text-white border-neutral-700 hover:bg-neutral-800">
                <Save className="w-4 h-4" />
                Save AI Settings
              </Button>
            </div>
